...
namespace Soft64.PI
{
    public sealed class BootRom : Soft64.PI.IBootRom
    {
        ...
        private UInt32 m_BootChecksum;

        internal BootRom(Stream romStream)
        {
            m_RomStream = romStream;
            ComputeCICType();
        }
        ...
        public void CopyCode(Stream dest)
        {
            m_RomStream.Position = CartRom.HeaderSize;
            Byte[] buffer = new Byte[Size];
            m_RomStream.Read(buffer, 0, buffer.Length);
            dest.Write(buffer, 0, buffer.Length);
        }

        public CICKeyType CIC
        {
            get { return m_CICType; }
        }

        public Int32 BootChecksum
        {
            get { return (Int32)m_BootChecksum; }
        }

        #region Crc32 Private Class

        private sealed class Crc32HashAlgorithm : HashAlgorithm
        {
            public const UInt32 DefaultSeed = 0xffffffff;
            private UInt32 crcValue = 0;

            private static readonly UInt32[] CrcTable = new UInt32[] {
